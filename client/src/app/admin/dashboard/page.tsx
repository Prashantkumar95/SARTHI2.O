...
function TabButton({
  ...
  return (
    ...
    >
      ...
      <span
        className={`min-w-[22px] h-5 px-1.5 text-[11px] font-bold rounded-full flex items-center justify-center
        ${
          active
            ? "bg-white text-black"
            : count > 0
...
function ContentList({ data, type }: ContentListProps) {
  ...
  async function startKyc(vendorId: string) {
    try {
      const res = await axios.patch(
        `/api/admin/vendors/video-kyc/start/${vendorId}`
      );
  ...
  return (
    <div className="space-y-3">
      {data.map((item, i) => {
        const name = item.name || item.ownerName || "Unknown";

        const email = item.email || item.ownerEmail || "No Email";
        ...
        const s =
          KYC_STATUS[item.videoKycStatus || "pending"] ||
          KYC_STATUS.pending;
        ...
        return (
          ...
          >
            ...
            <div className="flex items-center gap-3 min-w-0">
              <div
                className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold shrink-0 ${avColor}`}
              >
                {initials}
              </div>
              ...
              <div className="min-w-0">
                ...
                {type === "kyc" && (
                  <span
                    className={`mt-1.5 inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-0.5 rounded-full ${s.pill}`}
                  >
                    <span
            ...
            <div className="shrink-0">
              {type === "kyc" ? (
                item.videoKycStatus === "in_progress" ? (
                  ...
                  <button
                    onClick={() => startKyc(item._id)}
                    className="flex items-center gap-2 px-4 py-2 rounded-xl bg-black hover:bg-neutral-800 text-white text-sm font-semibold"
                  >
                    <Video size={13} />
                    Start KYC
                  </button>
...
function Kpi({
  ...
  return (
    <motion.div
      ...
      className={`bg-white rounded-2xl p-5 border border-gray-100 shadow-sm relative overflow-hidden ${cfg.cardHover}`}
    >
      <div className="flex items-start justify-between mb-4">
        <div
          className={`w-11 h-11 rounded-xl flex items-center justify-center ${cfg.iconBg} ${cfg.iconColor}`}
        ...
        {trend && (
          <span
            className={`inline-flex items-center gap-1 text-[11px] font-bold px-2 py-1 rounded-full ${trendColor}`}
          >
            {trendIcon}
            {trend}
          </span>
      ...
      <p className="text-3xl font-extrabold text-gray-950 leading-tight">
        {value}
      </p>
